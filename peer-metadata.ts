  ...
  try {
    ...
    if (receivedInfoHash.every((b, i) => b === infoHashBytes[i])) {
      ...
      if (extHandshake) {
        ...
        const m = (extHandshake as any).m as
          | Record<string, unknown>
          | undefined;
        const utMeta = typeof m?.["ut_metadata"] === "number"
          ? (m!["ut_metadata"] as number)
          : null;
        const metadataSize =
          typeof (extHandshake as any).metadata_size === "number"
            ? (extHandshake as any).metadata_size as number
            : null;
        ...
        if (
          utMeta && metadataSize && metadataSize > 0 &&
          metadataSize < 8 * 1024 * 1024
        ) {
          try {
            const meta = await fetchMetadataFromPeer(
              conn,
              utMeta,
              metadataSize,
              timeoutMs,
...
export async function getTorrentMetadata(
  magnet: string,
  ...
  return await firstSuccess(
    peers,
    CONCURRENCY,
    (peer) => tryConnect(parsed.infohash, peerId, peer, 4000),
  );
}

export async function getPeersFromParsedMagnet(
...
export async function firstSuccess<TInput, TOut>(
  items: TInput[],
  concurrency: number,
  worker: (item: TInput) => Promise<TOut | null>,
): Promise<TOut | null> {
...
export async function readExactly(
  ...
  while (off < n) {
    ...
    const readN: number = await withTimeout<number>(
      conn.read(toRead).then((v: number | null) => v ?? 0),
      timeoutMs,
      `read(${n - off})`,
    );
...
async function readBtMessage(
  conn: ConnLike,
  timeoutMs: number,
): Promise<BtMessage | null> {
...
export async function readUntilExtendedHandshake(
  conn: ConnLike,
  timeoutMs: number,
...
function bdecodePrefix(buf: Uint8Array, start = 0): DecodedWithPos<any> {
  ...
  function readInt(): number {
    pos++; // 'i'
    const s = pos;
    while (buf[pos] !== 0x65) pos++; // 'e'
    const numStr = td.decode(buf.subarray(s, pos));
    pos++;
  ...
  function readList(): any[] {
    pos++;
    const arr: any[] = [];
    while (buf[pos] !== 0x65) arr.push(readAny());
    pos++;
    return arr;
  }
  ...
  function readAny(): any {
    ...
    if (b === 0x69) return readInt();
    if (b >= 0x30 && b <= 0x39) return readString();
...
async function fetchMetadataFromPeer(
  ...
  while (Date.now() < deadline) {
    ...
    const msgType = typeof header?.msg_type === "number"
      ? header.msg_type as number
      : NaN;
    const pieceIndex = typeof header?.piece === "number"
      ? header.piece as number
      : NaN;
    if (
      msgType !== 1 /* data */ || Number.isNaN(pieceIndex) || pieceIndex < 0 ||
