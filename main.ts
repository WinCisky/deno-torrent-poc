...
async function tryConnect(peer: { ip: string, port: number }, timeoutMs = 2000) {
...
async function test() {
    ...
    for (const tracker of trackers) {
        const trackerPeers = parseTrackerPeers(tracker);
        if (!trackerPeers) continue;
        peers.push(...trackerPeers);
    }

    for (const peer of peers) {
        await tryConnect(peer);
    }
}
